...
function checkreg() {
	...
	if (document.regform.idcard.value.replace(/\s+$|^\s+/g,"").length<=0) {
		alert("\请输入您的身份证号！");
		document.regform.realname.focus();
		return false;
	}
	...
	if (document.regform.phone.value.replace(/\s+$|^\s+/g,"").length<=0) {
		alert("\请输入您的手机！");
  ...
  <TABLE align=center>
	...
	<TR>
		...
		<TD align="left"><input type="text" size="10" name="idcard"></TD>
	</TR>
	<TR>
	...
	<TR>
	  ...
	  <TD align="left"><input type="text" size="30" name="phone"></TD>
	</TR>
    <TR>
